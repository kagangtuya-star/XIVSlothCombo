    ...
    internal class SAM
    {
        ...
        internal class SAM_ST_SimpleMode : CustomCombo
        {
            ...
            protected override uint Invoke(uint actionID, uint lastComboMove, float comboTime, byte level)
            {
                ...
                if (actionID is Gekko)
                {
                    ...
                    if (CanWeave(actionID))
                    {
                        ...
                        //Senei Features
                        if (gauge.Kenki >= 25 && ActionReady(Senei) && HasEffect(Buffs.MeikyoShisui) && HasEffect(Buffs.Fugetsu) && HasEffect(Buffs.Fuka))
                            return Senei;

                        //Zanshin Usage
                        if (LevelChecked(Zanshin) && gauge.Kenki >= 50 && CanWeave(actionID) && HasEffect(Buffs.ZanshinReady) && ((meikyostacks is 2 && WasLastWeaponskill(Gekko)) || GetBuffRemainingTime(Buffs.ZanshinReady) <= 6))
                            return OriginalHook(Ikishoten);

                        if (LevelChecked(Shoha) && gauge.MeditationStacks is 3)
                            return Shoha;

                        if (LevelChecked(Shinten) && gauge.Kenki > 50 && !HasEffect(Buffs.ZanshinReady) && ((gauge.Kenki >= 50) || (enemyHP <= 1)))
                            return Shinten;
                    }
                    ...
                    if (HasEffect(Buffs.Fugetsu) && HasEffect(Buffs.Fuka))
                    {
                        ...
                        if (!IsMoving && LevelChecked(OgiNamikiri) && (gauge.Kaeshi == Kaeshi.NAMIKIRI || (HasEffect(Buffs.OgiNamikiriReady) && !HasEffect(Buffs.ZanshinReady) && ((meikyostacks is 2 && WasLastWeaponskill(Higanbana)) || GetBuffRemainingTime(Buffs.OgiNamikiriReady) <= 6))))
                        ...
                        if (LevelChecked(Iaijutsu))
                        {
                            ...
                            if (!IsMoving && ((oneSeal && GetDebuffRemainingTime(Debuffs.Higanbana) <= 10 && enemyHP > 1 && !LevelChecked(Traits.EnhancedMeikyoShishui2)) || (oneSeal && enemyHP > 1 && LevelChecked(Traits.EnhancedMeikyoShishui2) && !HasEffect(Buffs.Tendo) && WasLastWeaponskill(Gekko)) || (twoSeal && !LevelChecked(MidareSetsugekka)) || (threeSeal && LevelChecked(MidareSetsugekka)) || (threeSeal && LevelChecked(TendoSetsugekka) && !HasEffect(Buffs.KaeshiSetsugekkaReady))))
                    ...
                    if (comboTime > 0)
                    {
                        if (lastComboMove is Hakaze or Gyofu && LevelChecked(Jinpu))
                        {
                            ...
                            if ((!LevelChecked(Kasha) && ((GetBuffRemainingTime(Buffs.Fugetsu) < GetBuffRemainingTime(Buffs.Fuka)) || !HasEffect(Buffs.Fugetsu))) || (LevelChecked(Kasha) && (!HasEffect(Buffs.Fugetsu) || (HasEffect(Buffs.Fuka) && !gauge.Sen.HasFlag(Sen.GETSU)) || (threeSeal && (GetBuffRemainingTime(Buffs.Fugetsu) < GetBuffRemainingTime(Buffs.Fuka))))))
                                return Jinpu;

                            if (LevelChecked(Shifu) && ((!LevelChecked(Kasha) && ((GetBuffRemainingTime(Buffs.Fuka) < GetBuffRemainingTime(Buffs.Fugetsu)) || !HasEffect(Buffs.Fuka))) || (LevelChecked(Kasha) && (!HasEffect(Buffs.Fuka) || (HasEffect(Buffs.Fugetsu) && !gauge.Sen.HasFlag(Sen.KA)) || (threeSeal && (GetBuffRemainingTime(Buffs.Fuka) < GetBuffRemainingTime(Buffs.Fugetsu)))))))
                                return Shifu;
                        }
            ...
            private static bool UseTsubame()
            {
                ...
                if (LevelChecked(TsubameGaeshi))
                {
                    ...
                    {
                        //1 and 2 min
                        if ((MeikyoUsed is 0 or 1 or 2 or 3 or 4 or 9 or 10) && WasLastWeaponskill(MidareSetsugekka))
                            return true;

                        // 3 and 4 min
                        if ((MeikyoUsed is 5 or 6 or 11 or 12) && GetBuffStacks(Buffs.MeikyoShisui) is 2)
                            return true;

                        // 5 and 6 min
                        if ((MeikyoUsed is 7 or 8 or 13 or 14) && GetBuffStacks(Buffs.MeikyoShisui) is 1)
                            return true;
                    }
                }
                return false;
        ...
        internal class SAM_ST_AdvancedMode : CustomCombo
        {
            ...
            protected override uint Invoke(uint actionID, uint lastComboMove, float comboTime, byte level)
            {
                ...
                if (actionID is Gekko)
                {
                    ...
                    if (CanWeave(actionID))
                    {
                        ...
                        {
                            //Meikyo Features
                            if (IsEnabled(CustomComboPreset.SAM_ST_CDs_MeikyoShisui) && !HasEffect(Buffs.MeikyoShisui) && ActionReady(MeikyoShisui) && JustUsed(MidareSetsugekka, 1))
                                return MeikyoShisui;
                            ...
                            //Senei Features
                            if (IsEnabled(CustomComboPreset.SAM_ST_CDs_Senei) && gauge.Kenki >= 25 && ActionReady(Senei) && HasEffect(Buffs.MeikyoShisui) && HasEffect(Buffs.Fugetsu) && HasEffect(Buffs.Fuka))
                                return Senei;

                            //Zanshin Usage
                            if (IsEnabled(CustomComboPreset.SAM_ST_CDs_Zanshin) && LevelChecked(Zanshin) && gauge.Kenki >= 50 && CanWeave(actionID) && HasEffect(Buffs.ZanshinReady) && ((meikyostacks is 2 && WasLastWeaponskill(Gekko)) || GetBuffRemainingTime(Buffs.ZanshinReady) <= 6))
                                return OriginalHook(Ikishoten);
                    ...
                    {
                        //Ogi Namikiri Features
                        if (IsEnabled(CustomComboPreset.SAM_ST_CDs_OgiNamikiri) && (!IsEnabled(CustomComboPreset.SAM_ST_CDs_OgiNamikiri_Movement) || (IsEnabled(CustomComboPreset.SAM_ST_CDs_OgiNamikiri_Movement) && !IsMoving)) && LevelChecked(OgiNamikiri) && (gauge.Kaeshi == Kaeshi.NAMIKIRI || (HasEffect(Buffs.OgiNamikiriReady) && !HasEffect(Buffs.ZanshinReady) && ((meikyostacks is 2 && WasLastWeaponskill(Higanbana)) || GetBuffRemainingTime(Buffs.OgiNamikiriReady) <= 6))))
                            return OriginalHook(OgiNamikiri);
                        ...
                        if (IsEnabled(CustomComboPreset.SAM_ST_CDs_Iaijutsu) && LevelChecked(Iaijutsu))
                        {
                            ...
                            if ((!IsEnabled(CustomComboPreset.SAM_ST_CDs_Iaijutsu_Movement) || (IsEnabled(CustomComboPreset.SAM_ST_CDs_Iaijutsu_Movement) && !IsMoving)) && ((oneSeal && enemyHP > HiganbanaThreshold && ((!TraitLevelChecked(Traits.EnhancedMeikyoShishui2) && GetDebuffRemainingTime(Debuffs.Higanbana) <= 10) || (TraitLevelChecked(Traits.EnhancedMeikyoShishui2) && !HasEffect(Buffs.Tendo) && WasLastWeaponskill(Gekko)))) || (twoSeal && !LevelChecked(MidareSetsugekka)) || (threeSeal && (LevelChecked(MidareSetsugekka) || (LevelChecked(TendoSetsugekka) && !HasEffect(Buffs.KaeshiSetsugekkaReady))))))
                                return OriginalHook(Iaijutsu);
                        }
                    }
            ...
            private static bool UseTsubame()
            {
                ...
                if (IsEnabled(CustomComboPreset.SAM_ST_CDs_Iaijutsu) && LevelChecked(TsubameGaeshi))
                {
                    //Tendo
                    if (WasLastWeaponskill(TendoSetsugekka) && HasEffect(Buffs.TendoKaeshiSetsugekkaReady))
                        return true;
        ...
        internal class SAM_AoE_OkaCombo : CustomCombo
        {
            ...
            protected override uint Invoke(uint actionID, uint lastComboMove, float comboTime, byte level)
            {
                ...
                if (actionID is Oka)
                {
                    if (IsNotEnabled(CustomComboPreset.SAM_AoE_OkaCombo_TwoTarget) && gauge.Kenki >= Config.SAM_AoE_KenkiOvercapAmount && LevelChecked(Kyuten) && CanWeave(actionID))
                        return Kyuten;

                    if (IsNotEnabled(CustomComboPreset.SAM_AoE_OkaCombo_TwoTarget) && HasEffect(Buffs.MeikyoShisui))
                        return Oka;
        ...
        internal class SAM_Iaijutsu : CustomCombo
        {
            ...
            protected override uint Invoke(uint actionID, uint lastComboMove, float comboTime, byte level)
            {
                ...
                if (actionID is Iaijutsu)
                {
                    ...
                    if (IsEnabled(CustomComboPreset.SAM_Iaijutsu_OgiNamikiri) && ActionReady(OgiNamikiri) && (gauge.Kaeshi is Kaeshi.NAMIKIRI || HasEffect(Buffs.OgiNamikiriReady)))
                        return OriginalHook(OgiNamikiri);

                    if (IsEnabled(CustomComboPreset.SAM_Iaijutsu_TsubameGaeshi) && ActionReady(TsubameGaeshi) && (gauge.Kaeshi != Kaeshi.NONE || WasLastWeaponskill(TendoSetsugekka)))
                        return OriginalHook(TsubameGaeshi);
                }
                return actionID;
            }
