...
using static XIVSlothComboX.Combos.PvE.MCH;
        ...
        internal class MCHOpenerLogic96
        {
            ...
            private bool DoOpener(ref uint actionID)
            {
                ...
                if (currentState == OpenerState.InOpener)
                {
                    ...
                    if (((actionID == CheckMate && GetRemainingCharges(CheckMate) < 3) || (actionID == Chainsaw && IsOnCooldown(Chainsaw)) || (actionID == Wildfire && IsOnCooldown(Wildfire)) || (actionID == BarrelStabilizer && IsOnCooldown(BarrelStabilizer)) || (actionID == BarrelStabilizer && IsOnCooldown(Excavator)) || (actionID == DoubleCheck && GetRemainingCharges(DoubleCheck) < 3)) && ActionWatching.TimeSinceLastAction.TotalSeconds >= 3)
                    {
                        CurrentState = OpenerState.FailedOpener;
                        return false;
                    }
