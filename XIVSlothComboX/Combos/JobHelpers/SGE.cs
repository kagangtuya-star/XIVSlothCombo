...
using static XIVSlothComboX.Combos.PvE.SGE;
...
namespace XIVSlothComboX.Combos.JobHelpers
{
    internal class SGE
    {
        ...
        internal class SGEOpenerLogic
        {
            ...
            {
                if (GetRemainingCharges(发炎Phlegma3) < 2)
                    return false;

                if (!ActionReady(心神风息Psyche))
                    return false;
            ...
            private bool DoOpener(ref uint actionID)
            {
                ...
                if (currentState == OpenerState.InOpener)
                {
                    ...
                    if (WasLastAction(心神风息Psyche) && OpenerStep == 6) OpenerStep++;
                    else if (OpenerStep == 6) actionID = 心神风息Psyche;

                    if (WasLastAction(发炎Phlegma3) && OpenerStep == 7) CurrentState = OpenerState.OpenerFinished;
                    else if (OpenerStep == 7) actionID = 发炎Phlegma3;

                    if (ActionWatching.TimeSinceLastAction.TotalSeconds >= 5)
                        CurrentState = OpenerState.FailedOpener;

                    if (((actionID == 发炎Phlegma3 && GetRemainingCharges(发炎Phlegma3) == 0) || (actionID == 心神风息Psyche && IsOnCooldown(心神风息Psyche))) && ActionWatching.TimeSinceLastAction.TotalSeconds >= 3)
                    {
                        CurrentState = OpenerState.FailedOpener;
                ...
                switch (i)
                {
                    ...
                    case 2:
                        ...
                        enabled = IsEnabled(CustomComboPreset.SGE_ST_Heal_Pepsis)
                                  && FindEffect
                                  (
                                      Buffs.EukrasianDiagnosis, healTarget,
